...
export default observer(function ActivityDetailedChat({ activityId }: Props) {
    ...
    useEffect(() => {
        ...
        if (activityId) {
            commentStore.createHubConnecdtion(activityId);
        }
                    ...
                    >
                        {({ isSubmitting, isValid,handleSubmit }) => (
                            <Form className='ui form'>
                                <Field name='body'>
                                    {(props: FieldProps) => (
                                        <div style={{ position: 'relative' }}>
                                            ...
                                            <textarea placeholder='Enter your comments (Enter to submit, SHIFT + Enter for new line'
                                                rows={2}
                                                {...props.field}
                                                onKeyDown={e=>{
                                                    console.log(e.key);
                                ...
                                {/* <MyTextArea placeholder='Add commnet' name='body' rows={2} />
                                <Button
